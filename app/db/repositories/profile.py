...
from app.models.profile import UserProfileCreate, UserProfileUpdate, UserProfileInDB
...
class UserProfileRepository(BaseRepository):
    async def create_profile_for_user(self, *, profile_create:
                                      UserProfileCreate) -> UserProfileInDB:
        created_profile = await self.db.fetch_one(query=CREATE_PROFILE_FOR_USER_QUERY, values=profile_create.dict())

        return created_profile

    async def get_profile_by_user_id(self, *, user_id: int) -> UserProfileInDB:
        profile_record = await self.db.fetch_one(query=GET_PROFILE_BY_USER_ID_QUERY, values={"user_id": user_id})

        if not profile_record:
            return None

        return UserProfileInDB(**profile_record)

    async def get_profile_by_username(self, *, username: str) -> UserProfileInDB:
    ...
    async def update_profile(self, *, profile_update: UserProfileUpdate,
                             requesting_user: UserInDB) -> UserProfileInDB:
